...
| Celeritas | QUEUED | `celeritas/build.sh` / LUNARC copy `build-fixed-20260512.sh` | compact `celer-sim` simple-cms gamma workload | SLURM job 3041282 ran on `cg05` and failed with exit `127:0` because the script used a stale `app/celer-sim/celer-sim` executable path while the build produced `bin/celer-sim`; the corrected script copy was submitted as job 3047497 on `gpua40` and is pending resources. Full TestEm3/Hadr04/ZDC is still `OPEN:` until HepMC3/VecGeom/reference inputs are enabled. |

...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix; gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 13:48 CEST guarded LUNARC refresh (`ssh -O check`, then `squeue -j 3040180,3040259,3040275 --array`, `sacct -X`, and stdout tails) showed four RUNNING external blockers: 3040275_10, 3040259_5, 3040259_9, and 3040180_25; 3040259_4 has now completed successfully in sacct (05:12:41, exit 0:0). Elapsed/time-left from `squeue`: 3040275_10 05:05:22/06:54:38; 3040259_5 05:16:43/06:43:17; 3040259_9 05:16:43/06:43:17; 3040180_25 11:34:18/00:25:42. Fresh stdout tails reached 3040259_9 event ~822,767 and 3040275_10 event ~182,078; 3040180_25 and 3040259_5 remained RUNNING with stale/slow stdout (`slurm/cosmic-3040180_25.out` mtime 02:20 CEST, visible event 2553; `slurm/cosmic-3040259_5.out` mtime 08:34 CEST, visible event 357). Keep open until these finish, fail, or cancel; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| Lead-glass PMT calibration formula audit | NEXT | leadglass-pmt-calibration | See `docs/parallel-sessions/leadglass-pmt-calibration.md`; audit Ch. 5 `E_gamma = 0.46 N_PMT + 8.02` and reflectivity/calibration mismatches without retuning constants. |
| Scintillator WLS light-collection parameterization | NEXT | scintillator-wls-parameterization | See `docs/parallel-sessions/scintillator-wls-parameterization.md`; fail-closed audit for Ch. 5 WLS `f(r)`/`f(z)` evidence. |
| Electron-pair object integration | NEXT | electron-pair-integration | See `docs/parallel-sessions/electron-pair-integration.md`; integrate/audit Ch. 8 5 cm e+/e- pair handling while respecting file caps. |
...
| **Geant4 hot-path source review** | RUNNING | g4-source-review | Partial report underway at `docs/reports/g4_source_review_hotpaths.md`; iteration 1 covers PIL from upstream Geant4 tag `v11.2.2` after LUNARC install exposed no source tree. Top worker-3 follow-up after Phase 5 baseline: `g4gpu-phase5d-jit-poststep-gpil`. |
...
| **Supervisor 6-pane protocol refresh** | NEXT | supervisor-protocol-refresh | Review finding from `d1b5ec3`: prompts/Makefile launch worker-0..4 plus planner, but shared protocol still describes the legacy four-pane L0--L3 batch; see `docs/parallel-sessions/supervisor-protocol-refresh.md`. |

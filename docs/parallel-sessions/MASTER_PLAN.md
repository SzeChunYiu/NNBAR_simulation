...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix (3040180_0-11 failed and are retried by 3040259; 3040180_12-23 and 3040259_0,1,2,3,6,7,8,10,11 completed). Gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 12:29 CEST guarded LUNARC check (`ssh -O check` then `squeue -j 3040180,3040259,3040275 --array` and `sacct -X`) still showed the same six RUNNING blockers: 3040180_24,25; 3040259_4,5,9; 3040275_10. Elapsed times: 3040180_24/25 10:15:35, 3040259_4/5/9 3:58:00, 3040275_10 3:46:39. Fresh log tails reached 3040180_24~977,411, 3040259_4~759,665, 3040259_9~617,927, 3040275_10~135,192; high-energy 3040180_25 and 3040259_5 remained RUNNING with stale/slow stdout (`cosmic-3040180_25.out` mtime 02:20 CEST, visible event 2553; `cosmic-3040259_5.out` mtime 08:34 CEST, visible event 357). Keep open until these finish, fail, or cancel; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| Geometry manifest path portability fix | RUNNING | fix-geometry-manifest-paths | Planner review of `c8de07e` found hardcoded thesis-extract absolute paths in `geometry_constants.py`/tests; make source-path resolution portable or env-overridable before the manifest is reused on LUNARC. |
...
| TPC response and digitization boundary audit | NEXT | tpc-response-boundary-audit | See `docs/parallel-sessions/tpc-response-boundary-audit.md`; make thesis first-order TPC electron-count response and advanced/non-thesis digitization boundaries explicit and tested before reconstruction claims mix evidence levels. |
| Thesis reproduction ledger command/sample closure | NEXT | thesis-ledger-command-closure | See `docs/parallel-sessions/thesis-ledger-command-closure.md`; add a fail-closed audit of ledger commands and sample paths before any blocked thesis rows are cited as executable or reproduced. |

...
| Charged PID downstream unit cleanup | DONE | charged-pid-unit-cleanup | Planner review of `b48b31b` found `identify_particle_type` still documents charged-track `dedx` as MeV/cm and feeds the new e-/cm value into the Bethe-Bloch MeV/cm momentum estimator; clarify the legacy fallback or gate momentum estimation and add a focused regression. |

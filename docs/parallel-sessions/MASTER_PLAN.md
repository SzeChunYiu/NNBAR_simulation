...
| HIBEAM evidence test path portability fix | DONE | fix-hibeam-evidence-paths | Committed in worker-1: optional HIBEAM article evidence now comes only from `NNBAR_HIBEAM_ARTICLE_TEX` with skip-safe missing-file handling; forbidden machine-specific path regression covers the test/helper sources. HIBEAM evidence remains fail-closed when optional paper text is supplied. Verification passed: focused 6/1 skipped without env, 7/7 with env, full `tests/` 124 passed/2 skipped, forbidden-path grep no matches, file caps 173/341 lines. |

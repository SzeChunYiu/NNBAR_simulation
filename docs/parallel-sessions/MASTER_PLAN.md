...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix; gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 13:09 CEST guarded LUNARC check (`ssh -O check`, then `squeue -j 3040180,3040259,3040275 --array`, `sacct -X`, and stdout tails) still showed five RUNNING external blockers: 3040180_25, 3040259_4, 3040259_5, 3040259_9, and 3040275_10. Elapsed/time-left: 3040180_25 10:55:35/1:04:25; 3040259_4/5/9 04:38:00/7:22:00; 3040275_10 04:26:39/7:33:21. Fresh stdout tails reached 3040259_4 event ~890,330, 3040259_9 event ~721,935, and 3040275_10 event ~160,022; high-energy 3040180_25 and 3040259_5 remained RUNNING with stale/slow stdout (`slurm/cosmic-3040180_25.out` mtime 02:20 CEST, visible event 2553; `slurm/cosmic-3040259_5.out` mtime 08:34 CEST, visible event 357). Keep open until these finish, fail, or cancel; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| Carbon foil radius and source-vertex alignment | RUNNING | carbon-foil-alignment | See `docs/parallel-sessions/carbon-foil-alignment.md`; audit the thesis target/source-vertex convention against Geant4 geometry, particle-gun fallback, and Python config before code alignment. |
...
| HIBEAM GNN test path portability fix | RUNNING | fix-hibeam-gnn-paths | See `docs/parallel-sessions/fix-hibeam-gnn-paths.md`; review finding from `bd01ba9`: `tests/test_hibeam_gnn_feature_contract.py` hard-codes `/Volumes/MyDrive/nnbar/papers/overleaf-696757e2/main.tex`; replace with environment/fixture/skip-safe path handling and keep focused/full pytest green. |
| HIBEAM vertex method-comparison and metric-table closure | NEXT | hibeam-vertex-method-closure | See `docs/parallel-sessions/hibeam-vertex-method-closure.md`; audit Least-squares, Trackless, GraphNeT, and Clustering+GNN metrics across Compton levels with verified datasets, splits, uncertainties, and deployable/oracle status. |
| HIBEAM evidence archive and dataset-version pinning | DONE | hibeam-evidence-archive | Fail-closed audit helpers/tests added for dataset registry IDs, DEC links, validation reports, ledger rows, archive digests, commit/tag pins, and unresolved HIBEAM paper placeholders. Current local package remains blocked as intended until every claim has registry/DEC/ledger/report/archive-pin evidence. Verification passed: focused 5/5, full `tests/` 116/116 with 3 third-party deprecation warnings, file caps 341/128 lines. |

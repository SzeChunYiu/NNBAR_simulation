...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix; gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 12:47 CEST guarded LUNARC check (`ssh -O check`, then `squeue -j 3040180,3040259,3040275 --array` and `sacct -X`) showed five RUNNING external blockers: 3040180_25, 3040259_4, 3040259_5, 3040259_9, and 3040275_10; 3040180_24 has now COMPLETED (10:30:06, exit 0:0). Elapsed/time-left: 3040180_25 10:34:15/1:25:45; 3040259_4/5/9 04:16:40/7:43:20; 3040275_10 04:05:19/7:54:41. Fresh stdout tails reached 3040259_4 event ~820,335, 3040259_9 ~667,529, 3040275_10 ~147,029; high-energy 3040180_25 and 3040259_5 remained RUNNING with stale/slow stdout (`slurm/cosmic-3040180_25.out` mtime 02:20 CEST, visible event 2553; `slurm/cosmic-3040259_5.out` mtime 08:34 CEST, visible event 357). Keep open until these finish, fail, or cancel; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| Beam-induced background and TPC occupancy validation | NEXT | beam-background-tpc-occupancy | See `docs/parallel-sessions/beam-background-tpc-occupancy.md`; audit Appendix A absorber/background tables, current simulation surfaces, and the TPC 25 µs occupancy derivation before any beam-background claims are promoted. |
...
| Thesis reproduction ledger command/sample closure | RUNNING | thesis-ledger-command-closure | See `docs/parallel-sessions/thesis-ledger-command-closure.md`; add a fail-closed audit of ledger commands and sample paths before any blocked thesis rows are cited as executable or reproduced. |
| HIBEAM TPC GNN feature-contract and final-result closure | NEXT | hibeam-gnn-feature-contract | See `docs/parallel-sessions/hibeam-gnn-feature-contract.md`; audit TrackGNN/VertexGNN feature schema, Compton levels, split evidence, `sigma_r`, `epsilon`, uncertainties, and deployable/oracle labels before thesis or paper numbers are promoted. |

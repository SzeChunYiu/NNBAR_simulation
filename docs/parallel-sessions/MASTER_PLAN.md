...
| HIBEAM evidence archive and dataset-version pinning | RUNNING | hibeam-evidence-archive | See `docs/parallel-sessions/hibeam-evidence-archive.md`; add fail-closed checks for dataset registry IDs, decision-log links, validation reports, ledger rows, archive digests, and commit/tag pins before HIBEAM numbers are promoted. |

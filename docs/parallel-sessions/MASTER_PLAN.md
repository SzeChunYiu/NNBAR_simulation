...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix (3040180_0-11 failed and are retried by 3040259; 3040180_12-23 and 3040259_0,1,2,3,6,7,10,11 completed). Gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 10:26 CEST `squeue -j 3040180,3040259,3040275 --array` and `sacct` still showed remaining RUNNING blockers unchanged: 3040180_24,25; 3040259_4,5,8,9; 3040275_10. Latest log event maxima from `tail -5000`: 3040180_24~786k, 3040180_25~2.6k, 3040259_4~367k, 3040259_5~3.3k, 3040259_8~742k, 3040259_9~301k, 3040275_10~63k. Keep open until these finish; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| TPC dE/dx electron-count convention | RUNNING | tpc-dedx-electrons | Promoted from PROPOSED after scintillator-layer review: Ch.7 defines per-layer dE/dx as `N_e / Δx` with lower-60% truncation, while `compute_track_dedx` currently prefers `eDep/path_length`; see `docs/parallel-sessions/tpc-dedx-electrons.md`. |

...
| **CRY SLURM array (27/27 nonzero jobs submitted)** | RUNNING | cosmic-slurm-array | 27-bin matrix patch committed in nested `NNBAR_Detector` on `main` and `lane/cosmic-slurm-array` as `a344a47` (`fix(slurm): cover gamma cosmic bin 4`). Jobs 3040180/3040259 account for the original 26-job matrix (3040180_0-11 failed and are retried by 3040259; 3040180_12-23 and 3040259_0,1,2,3,6,7,8,10,11 completed). Gamma bin4 recovery (`particleIdx=1`, `energyBin=4`, 10-50 GeV, `N=2.30e7`) is job 3040275_10. 2026-05-11 12:24 CEST guarded LUNARC check (`ssh -O check` then `squeue -j 3040180,3040259,3040275 --array` and `sacct -X`) still showed six RUNNING blockers: 3040180_24,25; 3040259_4,5,9; 3040275_10. Elapsed times: 3040180_24/25 10:10:22, 3040259_4/5/9 3:52:47, 3040275_10 3:41:26. Fresh log tails reached 3040180_24~969,604, 3040259_4~742,818, 3040259_9~606,135, 3040275_10~132,337; high-energy 3040180_25 and 3040259_5 remained RUNNING with stale/slow stdout (`cosmic-3040180_25.out` mtime 02:20 CEST, visible event 2553; `cosmic-3040259_5.out` mtime 08:34 CEST, visible event 357). Keep open until these finish, fail, or cancel; do not duplicate gamma bin4 while 3040275_10 is RUNNING. |
...
| GEANT4 physics-list reproducibility audit | NEXT | geant4-physics-list-audit | See `docs/parallel-sessions/geant4-physics-list-audit.md`; document observed physics constructors, Geant4 version evidence, and FTFP_BERT versus FTFP_BERT_HP validation tags without running new simulations. |
...
| Geometry manifest path portability fix | NEXT | fix-geometry-manifest-paths | Planner review of `c8de07e` found hardcoded thesis-extract absolute paths in `geometry_constants.py`/tests; make source-path resolution portable or env-overridable before the manifest is reused on LUNARC. |
| TPC W-value policy audit | NEXT | tpc-w-value-policy-audit | See `docs/parallel-sessions/tpc-w-value-policy-audit.md`; expose production 23.6 eV versus 26.0--27.4 eV reference scaling as tested Python policy without editing over-cap calibration or C++ source files. |
| Final sensitivity and zero-survivor limit accounting | RUNNING | final-sensitivity-accounting | See `docs/parallel-sessions/final-sensitivity-accounting.md`; worker-1 is adding pure-Python weighted-yield and zero-survivor limit accounting without claiming unavailable exact-sample survival fractions. |

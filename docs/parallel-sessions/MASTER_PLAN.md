...
| Ch.4 detector geometry constants manifest | RUNNING | geometry-constants-manifest | See `docs/parallel-sessions/geometry-constants-manifest.md`; worker-1 is adding a Python manifest/audit for thesis detector constants versus reconstruction config, with C++ geometry treated read-only. |

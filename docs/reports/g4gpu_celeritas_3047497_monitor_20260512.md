...
## Refresh: 2026-05-12 12:26 CEST

The LUNARC socket had to be explicitly recycled after an SSH multiplexing
failure (`Session open refused by peer`); `/Users/billy/lunarc-init.sh` then
created and verified a fresh persistent socket before the read-only status
queries. Job 3047497 remains pending; no benchmark result exists and no new
job was submitted.

```text
REMOTE_DATE=2026-05-12 12:26:52 CEST
...
job 3047502 still pending on `Priority`; the `mcaccel-sup` supervisor holder
job was running on `cx08`. No Celeritas job has allocated a GPU node.
...
```text
REMOTE_DATE=2026-05-12 12:37:24 CEST
...
## Refresh: 2026-05-12 12:59 CEST

Read-only LUNARC refresh confirms job 3047497 is still scheduler-pending and
has not produced logs or benchmark output. No new job was submitted.

```text
REMOTE_DATE=2026-05-12 12:59:49 CEST
...
The active-job filter still shows only the Celeritas rerun 3047497 and the
historical V7 shakedown 3047502 as pending G4GPU jobs; three `mcaccel-sup`
holder jobs are running (`3047628` on `cx08`, `3047941` on `cx12`, and
`3047906` on `cx04`). No Celeritas job has allocated a GPU node.

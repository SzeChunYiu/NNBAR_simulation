...
- **Current implementation evidence:** plan 08 maps π⁰ pairing to
  `find_pi0_candidates` (`photon.py:204-260`). It declares
  the current output schema, forms unordered neutral photon pairs,
  computes opening angle, mass, energy sums, lead-glass fraction, and
  applies the six configured selection booleans inside the same
  function.
...
|---|---|---|---|
| `passes_mass_window` | 100 ≤ M ≤ 180 MeV | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |
| `passes_total_energy` | E ≤ 720 MeV | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |
| `passes_scintillator_energy` | Σ_scint ≤ 250 MeV | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |
| `passes_leadglass_energy` | Σ_LG ≤ 980 MeV | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |
| `passes_leadglass_fraction` | LG_fraction ≥ 0.55 | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |
| `passes_opening_angle` | α ≥ 30° | thesis Ch 8 | config fields in `ReconstructionConfig` (`reconstruction.py:14-56`); cut implemented by `find_pi0_candidates` (`photon.py:204-260`) |

Strict `passes_selection` = AND of all six.

Target candidates also carry diagnostic columns
(`selection_failure_reasons`, source-track aliases, charged-match
class, prompt timing) for validation; these are `@diagnostic_only` per
plan 01. The current split code only emits the strict
`passes_selection` boolean; TODO(L3): add the diagnostic-only columns
without changing production acceptance.
...
|---|---|---|---|---|---|
| P.5 | **All unordered pairs (current)** | Pair every neutral photon pair once by object-id ordering. | Current `find_pi0_candidates` implementation (`photon.py:204-260`; plan 08 §3.5.3). | Production-eligible; O(N²). | π⁰ efficiency, candidate multiplicity, runtime. |
...
| P.6 | **Six-cut selection (current)** | Apply mass, total-energy, scintillator-energy, lead-glass-energy, LG-fraction, and opening-angle cuts. | Config defaults in `ReconstructionConfig` (`reconstruction.py:14-56`); booleans in `find_pi0_candidates` (`photon.py:204-260`). | Production-eligible; thresholds must be thesis-cited or DEC-logged. | Signal efficiency, fake rate, N-1 sensitivity. |
| P.6 | **Prompt-timing veto** | Require photon time residuals near zero before accepting prompt π⁰s. | No prompt-timing field exists in the current split code; TODO(L3): add after timing calibration and DEC. | Eligible after timing calibration and DEC. | Accidental-rate reduction vs efficiency loss. |

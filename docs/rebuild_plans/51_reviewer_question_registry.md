...
| `RQ-L1-PI0-CUTS` | Are the six pi0 selection cuts decomposed into individual pass columns? | reproducibility | plans 34, 37, 47, 50 | per-cut fixture and Ch 8/Ch 10 ledger row | open until ledger/package artifact |
...
| `RQ-L1-STRANGE-BARYON` | Could K_S, Lambda, or Sigma production in beam-neutron material fake the signal? | physics | plans 14, 22, 44, 59, 50 | Lambda-enriched V0 rejection closure and residual interval | open until enriched slice exists |
| `RQ-L1-TOF` | Does timing add real cosmic rejection once detector resolution is included? | method | plans 36, 41, 45, 61, 50 | TOF ROC, nonzero resolution budget, and signal-loss interval | open until closure |
| `RQ-L1-BAYES-LIMIT` | Are low-count limits stable under Jeffreys vs flat Bayesian priors? | method | plans 04, 46, 64, 50 | prior-sensitivity table and plan-46 comparison ratios | open until statistics code lands |
...
- id: RQ-L1-SELECTION-CUTFLOW
  ...
  rerun_transcript_id: null
- id: RQ-L1-EM-P1-CLUSTERING
  asked_by: anticipated_examiner
  asked_on: 2026-05-10
  question: How do we know the neutral-shower clusterer is not using truth Track_ID grouping?
  category: realism
  affects_results: []
  routes_to_gate: [31, 38, 50, 57]
  status: open
  answer:
    summary: null
    artifact: null
...
- id: RQ-L1-EM-P2-DISCRIMINANT
  asked_by: anticipated_examiner
  asked_on: 2026-05-10
  question: Is the charged/neutral discriminator calibrated, or just a hard-cone reproduction rule?
  category: method
  affects_results: []
  routes_to_gate: [32, 38, 50, 57]
  status: open
  answer:
    summary: null
    artifact: null
...
- id: RQ-L1-PILEUP-L11
  asked_by: anticipated_examiner
  asked_on: 2026-05-10
  question: Can signal plus cosmic or beam pile-up change the quoted acceptance?
  category: systematics
  affects_results: []
  routes_to_gate: [1, 44, 45, 50, 58]
  status: open
  answer:
    summary: null
    artifact: null

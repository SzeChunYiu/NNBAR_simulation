...
| `RQ-L1-PI0-CUTS` | Are the six pi0 selection cuts decomposed into individual pass columns? | reproducibility | plans 34, 37, 47 | per-cut fixture and Ch 8/Ch 10 ledger row | open until ledger/package artifact |
| `RQ-L1-SELECTION-CUTFLOW` | Does the Ch 10 cut-flow use the current canonical singular `pass_*` columns? | reproducibility | plans 37, 47 | cut-flow identity table and cumulative-count artifact | open until ledger/package artifact |

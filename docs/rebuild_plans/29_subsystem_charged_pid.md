...
(`charged.py:149-226`, plan 08 §3.4) owns the current C.1/C.5
...
|---|---|---|---|
| Cut-based baseline | Existing `reconstruct_charged_objects` (`charged.py:149-226`) | Keep thesis Ch 8/9 threshold form but remove the C.1 truth-name gate before production scoring. | Reproduces baseline; ladder delta comes primarily from removing truth substitution. |

...
|---|---|---|
| compact current cluster/photon row surface | `reconstruct_photon_objects` resolves at `photon.py:60`, inside the cited `photon.py:60-201` range. | keep citation |
| energy-weighted centroid helper | `_weighted_centroid` resolves at `photon.py:28`, inside the cited `photon.py:28-48` range. | keep citation |

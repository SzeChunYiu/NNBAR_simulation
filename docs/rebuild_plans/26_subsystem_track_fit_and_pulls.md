...
- *Current.* `_track_anchor_and_direction` (plan 08 §3.2; `charged.py:61-80`): direction
...
|---|---|---|---|
| Current first-last direction | Existing `_track_anchor_and_direction` (`charged.py:61-80`) | Preserve as reproducibility baseline and degraded fallback when too few hits exist for a covariance fit. | No V.2 improvement; documents current truth-free but covariance-free baseline. |
